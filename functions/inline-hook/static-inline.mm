...
NSString* StaticInlineHookPatch(char* machoPath, uint64_t vaddr, char* patch)
{
    ...
    if(newPath) {
        ...
        if(!macho) return [NSString stringWithFormat:NSENC("找不到文件(can't find file):\n%@"), newPath];
    } else {
        macho = load_macho_data(path);
        if(!macho) return [NSString stringWithFormat:NSENC("无法读取文件(can't read file):\n%@"), path];
    }
    ...
    while(true) {
        ...
        if(!macho) {
            return NSENC("hujiu-add_hook_section error!");
        }
    }
    
    if(cryptid != 0) {
        return NSENC("hujiu-该app程序未砸壳!\nthis app is not decrypted!");
    }
    
    if(!text_seg || !data_seg) {
        return NSENC("hujiu-无法解析machO文件!\ncan not parse machO file!");
    }
    ...
    for(int i=0; i<STATIC_HOOK_CODEPAGE_SIZE/sizeof(StaticInlineHookBlock); i++)
    {
        ...
        if(hookBlock[i].hook_vaddr==0)
        {
            ...
            printf(ENC("found empty StaticInlineHookBlock %d %p=>%p\n"), i, targetRVA, targetData);
    ...
    if(!hookBlockRVA) {
        return NSENC("超过最大可用数量!\nHOOK count full!");
    }
    
    printf(ENC("func: %p=>%p target: %p=>%p\n"), funcRVA, funcData, targetRVA, targetData);
    ...
    NSString* savePath = [NSString stringWithFormat:NSENC("%@/Documents/static-inline-hook/%s"), NSHomeDirectory(), machoPath];
...
BOOL sub_1571(char* machoPath, uint64_t vaddr, char* patch)
{
    void* base = find_module_by_path(machoPath);
    if(!base) {
        return NO;
